...
import { 
  ...
  ChevronRight, Receipt, Wallet, HeartHandshake, X, Search, Check, UserPlus
} from 'lucide-react';
...
export default function MainDashboard({ onNavigateTab }) {
  ...
  const [attendanceRecords, setAttendanceRecords] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem(`attendance_${today}_Sunday 1st Morning Service (07:00 AM)`);
      return saved ? JSON.parse(saved) : {};
    } catch {
      return {};
    }
  ...
  // Safe Flat List of Believers
  const allBelievers = [];
  (families || []).forEach(fam => {
    if (fam?.headMember) allBelievers.push({ ...fam.headMember, familyName: fam.familyName });
    (fam?.members || []).forEach(m => allBelievers.push({ ...m, familyName: fam.familyName }));
  });

  // Handle Mark Attendance
  const handleMarkPresent = (id, name, type = 'Member') => {
    const today = new Date().toISOString().split('T')[0];
    const key = `attendance_${today}_Sunday 1st Morning Service (07:00 AM)`;
  ...
  // Handle Save New Visitor from Modal
  const handleSaveNewVisitor = (e) => {
    ...
    alert('புது விசுவாசி பதிவு செய்யப்பட்டு ஆராதனை வருகை குறிக்கப்பட்டது!');
  };
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12 w-full select-none text-slate-200">
      ...
      {/* 1. TOP HEADER & INTERACTIVE BRANCH FILTER */}
      <div className="glass-card rounded-3xl p-6 sm:p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-400 uppercase">Sunday Turnout</span>
        ...
        <div className="lg:col-span-2 glass-card rounded-3xl p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            <div>
              ...
              <p className="text-[11px] text-slate-400">வருகைப்பதிவு செய்ய உடனே மார்க்கரைத் திறக்கவும்</p>
            </div>

            {/* Launch Marker Pop-up Button */}
            <button
              type="button"
          ...
          <div className="grid grid-cols-2 gap-4 text-center">
            <div className="p-4 rounded-2xl bg-slate-900 border border-emerald-500/20">
              <span className="text-xs text-slate-400 font-bold">Checked In Today</span>
            ...
            >
              <span className="text-xs text-slate-400 font-bold">Total Seekers / Visitors</span>
          ...
          <div className="w-full max-w-lg p-6 rounded-3xl bg-[#0e1322] border border-white/20 shadow-2xl space-y-4">
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <CalendarCheck className="text-orange-400" size={18} />
                <span>Quick Service Attendance Marker</span>
              </h3>
            ...
            {/* TAB 1: EXISTING BELIEVERS SEARCH & MARK */}
            {modalTab === 'existing' ? (
              <div className="space-y-3">
                ...
                <div className="max-h-60 overflow-y-auto divide-y divide-white/5 pr-1">
                  {allBelievers
                    .filter(b => b.name?.toLowerCase().includes(searchMember.toLowerCase()) || (b.phone && b.phone.includes(searchMember)))
                    .map(b => {
                      const isMarked = attendanceRecords[b.memberId]?.status === 'Present';
                      return (
                        <div key={b.memberId} className="flex items-center justify-between py-2">
                          <div>
                            <div className="text-xs font-bold text-white">{b.name}</div>
                            <div className="text-[10px] text-slate-400">{b.familyName} • {b.phone}</div>
                          </div>
                          ...
                          <button
                            ...
                            className={`px-3 py-1 rounded-lg text-xs font-bold cursor-pointer transition-all ${
                              isMarked ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
                            }`}
                          >
              ...
              /* TAB 2: NEW VISITOR FORM */
              <form onSubmit={handleSaveNewVisitor} className="space-y-3">
                <div>
                  <label className="text-xs text-slate-300 font-medium">New Believer Name *</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-slate-900 border border-white/10 rounded-xl px-3.5 py-2 text-xs text-white mt-1 focus:outline-none focus:border-rose-500"
                  />
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    type="submit"
                    className="w-full py-2 bg-gradient-to-r from-[#ff6b00] to-[#f43f5e] text-white rounded-xl text-xs font-bold shadow-lg cursor-pointer"
                  >
                    Save & Mark Present
                  </button>
                </div>
